...
def load_tp(wb, conn, col_date):
    ...
    for row_num, r in enumerate(ws.iter_rows(min_row=2), start=2):
        ...
        for block in range(80):
            ...
            try:
                upsert_tp(conn, col_date, 날짜, sn, 시료명, 구분, 분석자,
                          시료량, 흡광도, 희석, 검량선a, 농도, 결과,
                          st_mgl, st_abs, curve)
                loaded += 1
                if loaded % 200 == 0:
                    conn.commit()
                    print("    [TP] %d건 처리 중... (행%d)" % (loaded, row_num))
            except Exception as e:
                print("    오류 행%d 블록%d: %s" % (row_num, block, e))

    conn.commit()
    print("  [TP] %d건 저장, %d건 스킵" % (loaded, skipped))
    return loaded
