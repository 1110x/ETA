#!/usr/bin/env python3
"""
청하 XLSM N-Hexan-DATA → 노말헥산_추출물질_시험기록부
"""
...
try:
    ...
    ws = wb['N-Hexan-DATA']
    print(f"✓ N-Hexan-DATA 시트 로드 (max_row: {ws.max_row})")
    ...
    if records:
        ...
        for rec in records:
            try:
                query = """
                    INSERT IGNORE INTO `노말헥산_추출물질_시험기록부`
                ...
                cursor.execute(query, (
                    ...
                    rec['등록일시'], rec['시료량'], rec['전무게'], rec['후무게'],
                    rec['무게차'], rec['희석배수'], rec['결과']
                ))
                inserted += 1
