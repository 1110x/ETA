...
public record WaybleCredential(string UserId, string Password, bool AutoLogin);

public static class WaybleCredentialService
{
    private static string FilePath
    {
        get
        {
            ...
            return Path.Combine(dir, "wayble_credential.dat");
        }
    }

    public static void Save(string userId, string password, bool autoLogin)
    {
        try
        {
            var payload = new WaybleCredential(userId, password, autoLogin);
    ...
    public static WaybleCredential? Load()
    {
        try
        {
            ...
            return JsonSerializer.Deserialize<WaybleCredential>(Decrypt(cipher));
        }
        catch { return null; }
    }
    ...
    {
        var seed = Environment.UserName + "|" + Environment.MachineName + "|ETA-WAYBLE-v1";
