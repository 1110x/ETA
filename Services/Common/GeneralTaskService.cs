...
public static class GeneralTaskService
{
    ...
    public static List<GeneralTask> GetAll()
    {
        EnsureTable();
        var list = new List<GeneralTask>();
        using var conn = DbConnectionFactory.CreateConnection();
        conn.Open();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = "SELECT * FROM `일반업무` ORDER BY Id DESC";
        using var r = cmd.ExecuteReader();
        while (r.Read()) list.Add(Read(r));
        return list;
    }

    public static List<GeneralTask> GetByAgent(string 담당자id)
    {
        EnsureTable();
        var list = new List<GeneralTask>();
        using var conn = DbConnectionFactory.CreateConnection();
        conn.Open();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = "SELECT * FROM `일반업무` WHERE 담당자id=@id ORDER BY Id DESC";
    ...
    public static void Insert(GeneralTask t)
    {
        EnsureTable();
        using var conn = DbConnectionFactory.CreateConnection();
        conn.Open();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = @"
            INSERT INTO `일반업무` (업무명, 내용, 배정자, 담당자id, 담당자명, 상태, 마감일, 등록일시)
            VALUES (@업무명, @내용, @배정자, @담당자id, @담당자명, @상태, @마감일, @등록일시)";
        SetParams(cmd, t);
        cmd.ExecuteNonQuery();
    }

    public static void Update(GeneralTask t)
    {
        EnsureTable();
        using var conn = DbConnectionFactory.CreateConnection();
        conn.Open();
    ...
    public static void Delete(int id)
    {
        ...
        conn.Open();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = "DELETE FROM `일반업무` WHERE Id=@Id";
        cmd.Parameters.AddWithValue("@Id", id);
        cmd.ExecuteNonQuery();
    }

    private static void SetParams(DbCommand cmd, GeneralTask t)
    {
    ...
    private static GeneralTask Read(DbDataReader r) => new()
    {
